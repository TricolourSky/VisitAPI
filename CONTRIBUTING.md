...
- **The `.dlg` syntax and the pack layout are frozen.** Changes to the script grammar, the dialogue id scheme or what counts as a content pack break every existing script, pack and save file; they need an issue and a maintainer's go-ahead first. Both live in the [VisitAPI Editor](https://github.com/TricolourSky/VisitAPI-Editor) repository (`src/VisitAPI.Dlg`, `src/VisitAPI.Packs`) and are compiled into this plugin from source — change them there, once, for both sides.
  **`.dlg` 语法和内容包布局是冻结契约。** 改语法、对话 id 算法或「什么算一个包」会让现有剧本、包和存档全部失效，先开 issue 讨论并得到维护者同意。这两样的源码住在 [VisitAPI Editor](https://github.com/TricolourSky/VisitAPI-Editor) 仓库（`src/VisitAPI.Dlg`、`src/VisitAPI.Packs`），本插件链源码编进来——改在那边，一次改两边。
...
See the "Build from source" section of the README. You need a local SPT 4.1.x install and the VisitAPI Editor repository next to this one (folder name `VisitAPI Editor`, or pass `-p:DlgSrc=` / `-p:PacksSrc=`).

见 README 的「从源码构建」。需要本机 SPT 4.1.x 安装，以及与本仓库并列的 VisitAPI Editor 仓库（文件夹名 `VisitAPI Editor`，或用 `-p:DlgSrc=` / `-p:PacksSrc=` 指过去）。
