...
/// <summary>VisitAPI 自己的任务横幅（非章节任务用）：借原生通知底盘（立起/躺下动画 + 音效 + 排队），
/// 只把底图换成从正式服截图切出来的那条。底图不调色——正式服底色本来就是 (24,24,24)，状态靠文字颜色区分；
/// 图标也不换，用游戏自己的任务通知图标，免得风格出戏。</summary>
public class VisitBanner : NotificationWithText
{
    // 底图整张水平翻转过：尖头在右边，左缘是干净直边——图标和文字不用让位；九宫格不拉伸边跟着换到右边（左4/上下8/右33=尖头段）
    ...
    // 底图在就原样显示、只压半透明（alpha 跟原生默认通知一致 200/255）；art 缺失时留 null 走原生默认色
    ...
    public override BaseNotificationView CreateView(INotificationViewFactory viewFactory)
    {
        ...
        // 默认横幅是池化复用的（NotifierView 的 POOL_SIZE=4）。Init 每次只重设 _icon.sprite 和 _background.color，
        // 底图和 Image.type 不重设——不还原就会串到 SPT 自己的通知上。
