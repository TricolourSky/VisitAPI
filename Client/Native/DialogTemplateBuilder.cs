...
public static class DialogTemplateBuilder
{
    ...
    public static MongoID Register(DialogTree tree, string entryNode, string startNode, string playerName, string profileId, QuestController quests)
    {
        ...
        foreach (var node in tree.Nodes.Values)
        {
            ...
            for (var i = 0; i < node.Options.Count; i++)
            {
                ...
                if (o.SetVarName != null) VariableService.Register(lineId, o.SetVarName, o.SetVarValue);
                ...
                open |= trigger == null;
                rows.Add(Line(lineId, EDialogSide.Player, OptionMap.Icon(o), QuestGates.Actions(o, OptionMap.Act(tree, startNode, o.Target)), trigger, Key(loc, tree.TraderId, node.Name, "opt" + i, o.Text, playerName)));
            }
            ...
            // 作者留了 `always` 出口的节点 open 为真，一个字都不会多出来。DEV_NOTES #80。
    ...
    /// 一个节点的"拍子"：旁白 / 台词 / 「继续…」按 NpcSlot 排好序，各自登记成对话模板（#65 旁白顺序）
