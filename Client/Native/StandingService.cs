...
public static class StandingService
{
    ...
    static void Apply(Profile profile, TraderScreensGroup screen, string traderId, double delta)
    {
        ...
        var json = "{\"traderId\":\"" + traderId + "\",\"delta\":" + delta.ToString(CultureInfo.InvariantCulture) + "}";
        Task.Run(() => RequestHandler.PostJson("/visitapi/standing/add", json))
            .ContinueWith(t => { if (t.IsFaulted) Plugin.Log.LogWarning("[standing] server sync failed: " + t.Exception?.GetBaseException().Message); });
    }
}
