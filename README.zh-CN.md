...
- **分支对话**：旁白、`{player}` 占位符、背景**图片或视频**（视频可循环或只播一遍）。
- **突袭内触发器** —— 走到地图某个点位即可交互。
...
- **`.dlg` 脚本参考** —— [docs/DLG_FORMAT.zh-CN.md](docs/DLG_FORMAT.zh-CN.md)（[English](docs/DLG_FORMAT.md)）。发布包不带 `docs/`——请在源码仓库阅读：<https://github.com/TricolourSky/VisitAPI>
